...
elements playbin actually plugged (to confirm v4l2h264dec feeds kmssink with no
CPU videoconvert in between), then shows a still image, then stops.
"""
import os
import sys
import time
...
VIDEO = sys.argv[1] if len(sys.argv) > 1 else \
    "/home/david/mediaplayer/media/01._Origo_Solutions_Grand_Opening_4K_1.mp4"
...
def dump_pipeline(pb):
    ...
    hw = any("v4l2" in n for n in names)
    conv = any(n in ("videoconvert", "videoscale") for n in names)
    print("  hardware decoder present:", hw)
    print("  CPU videoconvert/scale present:", conv,
          "(want False for zero-copy)" if conv else "")
...
def main():
    ...
    if IMAGE:
        ...
        p.load(IMAGE, kind="image", image_dur=4)
        time.sleep(5)
