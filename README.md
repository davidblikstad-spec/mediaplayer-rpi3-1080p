...
(no desktop), playing directly to DRM/KMS via **mpv**.

## Features

- **Web UI with login** (username + password, stored hashed; session cookie).
- **Fullscreen HDMI playback** of videos and images via mpv on DRM/KMS.
...
This installs a systemd service that takes over **tty1** (the HDMI console) so
mpv can drive the display, and starts it on boot. Then browse to
`http://<pi-ip>:8080` and create your admin account on first visit.

> The service runs a login session on tty1 (`PAMName=login`) so it becomes the
> active seat session — required for mpv to get DRM master and output to HDMI.
...
```
Note: run from the **active console**, not over SSH — mpv needs DRM master to
show video, so video output and the live snapshot only work on the console
...
- **In-browser preview** plays the original file; the browser must support the
  ...
  but still play fine on HDMI via mpv).

## Layout

```
app/            Flask app: config, media, mpv engine, cec, scheduler, routes
